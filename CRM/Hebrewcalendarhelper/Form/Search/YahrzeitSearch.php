 ...
 */
class CRM_Hebrewcalendarhelper_Form_Search_YahrzeitSearch extends CRM_Contact_Form_Search_Custom_Base implements CRM_Contact_Form_Search_Interface {
  protected $_formValues;
  protected $hebCal = null;
  protected $_systemDateFormat = "";

  function __construct(&$formValues) {
    require_once( 'utils/HebrewCalendar.php');
    $this->hebCal = new HebrewCalendar();
    ...
    $this->_columns = [
      ...
      ts('Deceased First Name') => 'deceased_first_name',
      ts('Deceased Last Name') => 'deceased_last_name',
      ts('ben/bas') => 'deceased_ben_bas',
      ts('Hebrew Name of Father') => 'father_hebrew_name',
      ts('Hebrew Name of Mother') => 'mother_hebrew_name',
  ...
  function buildForm(&$form) {
    ...
    $relative_time_interval_type_choices = array(
      '' => '-- select --' ,
...
*/
  ...
  //$hebyearmonth_fieldset = $form->addElement('hebyearmonth_fieldset')->setLabel('Choose Hebrew Year and Month');
   ...
   // $form->add('group', 'fieldset_hebmonthyear');
    ...
    $h_format_str = 'yy' ;
    $cur_hebrew_year = $this->hebCal->util_convert_today2hebrew_date($h_format_str);
    ...
    $tmp_hebrew_year_select = $form->add('select', 'hebrew_year_choice', ts('Hebrew Year'), $hebrew_years, false);
    ...
    foreach($heb_month_numbers as $cur ){
      ...
      $tmp_month_name = $this->hebCal->util_get_hebrew_month_name( $tmp, $tmp_hebrew_date);
      if( $cur == 6){
  ...
  function all($offset = 0, $rowcount = 0, $sort = null, $includeContactIDs = false, $onlyIDs = false) {
    ...
    $extended_religious_table = $this->hebCal->getSQLTableName(HebrewCalendar::RELIGIOUS_CUSTOM_FIELD_GROUP_NAME);
    ...
    $result = civicrm_api3('CustomGroup', 'get', array(
      ...
      'extends' => "Individual",
        ...
        $extended_mother_hebrewname = civicrm_api3('CustomField', 'getsingle', [
          'custom_group_id' => HebrewCalendar::RELIGIOUS_CUSTOM_FIELD_GROUP_NAME,
          'name' => HebrewCalendar::HEB_NAME_MOTHER_FIELD_NAME,
        ])['column_name'];

        $extended_father_hebrewname = civicrm_api3('CustomField', 'getsingle', [
          'custom_group_id' => HebrewCalendar::RELIGIOUS_CUSTOM_FIELD_GROUP_NAME,
          'name' => HebrewCalendar::HEB_NAME_FATHER_FIELD_NAME,
        ])['column_name'];
    ...
    $heb_name_sql = (!empty($extended_hebrewname) ? $extended_hebrewname." as hebrew_name, " : '');

    if (!empty($extended_plaque_location) && !empty($extended_has_plaque)) {
    ...
    $full_select = " mourner_contact_id as contact_id, mourner_contact_id as id, mourner_name as sort_name,
      ...
      contact_deceased.first_name as deceased_first_name, contact_deceased.last_name as deceased_last_name,
      CASE contact_deceased.gender_id
        WHEN 1 THEN 'bas'
	WHEN 2 THEN 'ben'
        ELSE ''
      END as deceased_ben_bas,
      $extended_religious_table.$extended_mother_hebrewname as mother_hebrew_name,
      $extended_religious_table.$extended_father_hebrewname as father_hebrew_name,
  ...
  function from() {
    ...
    $extended_religious_table = $this->hebCal->getSQLTableName(HebrewCalendar::RELIGIOUS_CUSTOM_FIELD_GROUP_NAME);
    $extended_plaque_table = $this->hebCal->getSQLTableName(HebrewCalendar::PLAQUE_CUSTOM_FIELD_GROUP_NAME);
  ...
  */
    ...
    $tmp_mem_join = "";
    $tmp_mem_types_of_contact = [];
    $tmp_mem_orgs_of_contact = [];

    if (!empty($this->_formValues['membership_type_of_contact'])) {
      $tmp_mem_types_of_contact = $this->_formValues['membership_type_of_contact'];
    }

    if (!empty($this->_formValues['membership_org_of_contact'])) {
      $tmp_mem_orgs_of_contact = $this->_formValues['membership_org_of_contact'];
    }

    if (!empty($tmp_mem_types_of_contact) || !empty($tmp_mem_orgs_of_contact)) {
  ...
  //  $tmp_rel_type_ids = "7, 6 ";   // Household member of , Head of Household
    ...
    $religious_sql = (!empty($extended_religious_table) ? " LEFT JOIN $extended_religious_table ON contact_deceased.id = $extended_religious_table.entity_id " : '');
    $plaque_sql = (!empty($extended_plaque_table) ? " LEFT JOIN $extended_plaque_table extra_plaque ON contact_deceased.id = extra_plaque.entity_id " : '');
  ...
  function getAllDateFiltersFromForm($clauses) {
    ...
    if (  $date_range_ui  == "hebrew_month_year" ){
       ...
       $rel_time_str =  $this->hebCal->get_yahrzeit_relative_date_sql($date_sql_field_name, $interval_type, $interval_count);
